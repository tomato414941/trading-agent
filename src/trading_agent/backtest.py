"""Backtest engine — simulate RSI strategy on historical data."""
...
def run_backtest(
    ...
    # Sharpe ratio (annualized, assuming hourly for 1h timeframe)
...
if __name__ == "__main__":
    ...
    print(f"Running backtest: {symbol} {timeframe} ({limit} candles)...\n")
    result = run_backtest(symbol, timeframe, limit)
    print(result.summary())

    if result.trades:
        print(f"\n--- Trade Log ({len(result.trades)} entries) ---")
        for t in result.trades:
            pnl_str = f"  PnL: ${t['pnl']:+.2f}" if t["pnl"] is not None else ""
            print(f"  {t['timestamp']}  {t['side'].upper():4s}  {t['qty']:.6f} @ ${t['price']:.2f}{pnl_str}")
